    ...
    <div class="x_panel" style="height: 500px !important">
      ...
      <div class="">
        ...
        @foreach ($belum as $data)
          <li><p>Pasien: {{ $data->pasien->nama }} | Dokter: {{ $data->dokter->nama }} <a href="{{url('/loket/DetailResep/dokter_id='. $data['dokter_id'] .'&pasien_id='.$data['pasien_id'].' ')}}" class="btn btn-flat btn-xs btn-info pull-right">Lihat</a><span class="pull-right" style="margin-right: 3px">{{$data->created_at->diffForHumans()}} | </span></p></li>
        @endforeach
        </ul>
      </div>
      </div>
